...
parser.add_argument("--annot_file", default="../ava_train_v1.0.csv",
                    help="Anotation file path.")
parser.add_argument("--actionlist_file",
                    default="../ava_action_list_v1.0.pbtxt",
                    help="Action list file path.")
parser.add_argument("--output_dir", default="out", help="Output path.")
parser.add_argument("--partial_process", type=bool, default=False,
                    help="Set True to process only part of the annotations.")
parser.add_argument("--partial_start", type=int, default=0,
                    help="Partial process starting index.")
parser.add_argument("--partial_num", type=int, default=0,
                    help="Partial process number.")
...
# video_id, middle_frame_timestamp, person_box, action_id
# e.g. -5KQ66BBWC4,0904,0.217,0.008,0.982,0.966,12

if FLAGS.partial_process:
    assert FLAGS.partial_num > 0, \
        "Partial process number should be positive."
    assert FLAGS.partial_start < len(annots), \
        "Start index is larger than annotation number!"
    annot_range = range(FLAGS.partial_start,
                        min(FLAGS.partial_start + FLAGS.partial_num,
                            len(annots)))
else:
...
for annot_number in annot_range:
    ...
    outpath = os.path.join(outdir_keyframe,
                           '%d.jpg' % (int(middle_frame_timestamp)))
    ...
    clip_start = middle_frame_timestamp - clip_time_padding - \
                 float(clip_length) / 2
    if clip_start < 0:
        clip_start = 0
    clip_end = middle_frame_timestamp + float(clip_length) / 2

    outpath_clip = os.path.join(outdir_clip, '%d.%s' % (
        int(middle_frame_timestamp), video_extension))
    ...
    outpath_svg = os.path.join(outdir_keyframe_svg,
                               '%d_%s_%d.svg' % (int(middle_frame_timestamp),
                                                 action_id, annot_number))
    ...
    svg_document.add(
        svg_document.rect(insert=(insert_left, insert_top),
    ...
    outpath_visu_jpg = os.path.join(
        outdir_visu_jpg,
        '%d_%s_%d.jpg' % (int(middle_frame_timestamp), action_id, annot_number))

    subprocess.call('convert -- "%s" "%s"' % \
                    (outpath_svg, outpath_visu_jpg), shell=True)
